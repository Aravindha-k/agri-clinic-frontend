...

{
  ...
  assert(liveSrc.includes("MUTED_MARKER_OPACITY = 0.85"), "offline styling keeps opacity >= 0.85");
  ...
  assert(liveSrc.includes("LiveEmployeeMapPopup"), "click popup shows employee details");
  assert(liveSrc.includes("key={markerKey}") || liveSrc.includes("key={String(userId)}"), "one marker keyed by user ID");
