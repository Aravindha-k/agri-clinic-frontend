...
export default function MasterCrops() {
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            toast("Failed to load crops", "error");
        } finally {
            setLoading(false);
        }
    }, [toast]);
    ...
    const handleSave = async (e) => {
        ...
        const payload = { name: fd.get("name"), code: fd.get("code"), category: fd.get("category"), is_active: fd.get("is_active") === "on" };
        try {
            setSaving(true);
            if (panel.mode === "edit") {
                await updateCrop(panel.item.id, payload);
                toast("Crop updated");
            } else {
    ...
    const handleDelete = async () => {
        try {
            setSaving(true);
            await deleteCrop(confirm.item.id);
            toast("Crop deleted");
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="p-4 border-b border-gray-100">
                    <div className="relative max-w-sm">
                        ...
                        <input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="Search crops…" className="w-full pl-10 pr-4 py-2 text-sm rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition" />
                    </div>
                </div>
            ...
            <SlidePanel open={panel.open} onClose={() => setPanel({ open: false, mode: "add", item: null })} title={panel.mode === "edit" ? "Edit Crop" : "Add Crop"}>
                <form onSubmit={handleSave} className="space-y-5">
                    ...
                    <div>
                        ...
                        <input name="code" defaultValue={panel.item?.code || ""} className="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Category</label>
                        <input name="category" defaultValue={panel.item?.category || ""} placeholder="e.g. Cereal, Pulse, Oilseed…" className="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none" />
                    </div>
            ...
            <ConfirmDialog open={confirm.open} title="Delete Crop" message={`Delete "${confirm.item?.name}"? This cannot be undone.`} onConfirm={handleDelete} onCancel={() => setConfirm({ open: false, item: null })} loading={saving} />
        </div>
    );
}
